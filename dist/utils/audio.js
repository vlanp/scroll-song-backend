...
const parseAudio = async (audio) => {
    ...
    let pictureMimeType;
    if (!picture) {
        const defaultPicturePath = path.join(__dirname.replace("dist", "src"), "../assets/default-cover.jpg");
    ...
    const pictureBase64 = `data:${pictureMimeType};base64,${pictureBuffer.toString("base64")}`;
    return {
        title,
        startTime_s,
        endTime_s,
